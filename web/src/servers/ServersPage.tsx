...
export function ServersPage() {
  ...
  return <section className="workspace-page">
    ...
    {selected && <ServerDetail server={selected} connection={connectionByID.get(selected.connection_id)} onClose={closeDetail} onAction={setConfirmation} />}
...
function ServerDetail({ server, connection, onClose, onAction }: { server: Server; connection?: ProviderConnection; onClose(): void; onAction(action: PowerAction): void }) {
  ...
  return <div className="detail-backdrop" onClick={onClose}><aside className="detail-panel" role="dialog" aria-label="服务器详情" onClick={(event) => event.stopPropagation()}>
    ...
    <section><h4>控制操作</h4><div className="future-actions">{powerActions.map((action) => {
    ...
    })}<button disabled>控制台 · 里程碑 4</button></div></section>
  </aside></div>
}
