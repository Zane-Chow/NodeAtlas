...
function Dashboard() {
  return <section className="content-grid">
    <article className="welcome-card"><p className="eyebrow">客户凭据模式</p><h2>统一管理多个服务商的服务器</h2><p>AWS、GCP 使用客户账户内的专用身份，VirtFusion 与 Virtualizor 使用客户 API，支持多账户清单、电源操作和安全控制台回退。</p><div className="progress-track"><span style={{ width: '100%' }} /></div></article>
    <article className="status-card"><span className="status-icon">PV</span><div><small>Provider 核心</small><strong>仅使用客户账户凭据</strong><p>AWS · GCP · VirtFusion User API · Virtualizor Enduser API</p></div></article>
